...
END
GO
/****** Object:  StoredProcedure [tsc].[CreateStateChangeTableTriggers]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [tsc].[CreateStateChangeTableTriggers]
	 @schemaname as varchar(128)
	,@tablename as varchar(128)
AS 
BEGIN
	...
	*****************************/
	set @tmpsql = ''
	set @tmpsql = @tmpsql + ';CREATE TRIGGER tscUpdateChange_' + @tablename + ' ON ' + @change + ' FOR INSERT AS '
	...
	*****************************/
	set @tmpsql = ''
	set @tmpsql = @tmpsql + ';CREATE TRIGGER tscInsertChange_' + @tablename + ' ON ' + @state + ' FOR INSERT AS '
	set @tmpsql = @tmpsql + ';BEGIN TRANSACTION '
	...
	*****************************/
	set @tmpsql = ''
	set @tmpsql = @tmpsql + ';CREATE TRIGGER tscInsertState_' + @tablename + ' ON ' + @source + ' FOR INSERT AS '
	set @tmpsql = @tmpsql + ';BEGIN TRANSACTION '
	...
	*****************************/
	...
	set @tmpsql = ''
	set @tmpsql = @tmpsql + ';CREATE TRIGGER tscDeleteState_' + @tablename + ' ON ' + @source + ' FOR DELETE AS '
	set @tmpsql = @tmpsql + ';BEGIN TRANSACTION '
...
END
GO
/****** Object:  StoredProcedure [tsc].[HoldStateChange]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [tsc].[HoldStateChange] 
	@tablename as varchar(128)
AS 
BEGIN 
/*
...
END
GO
/****** Object:  StoredProcedure [tsc].[ManifestStateChange]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [tsc].[ManifestStateChange] 
	 @schemaname as varchar(128)
	,@tablename as varchar(128)
AS 
BEGIN
	...
	*****************************/
	...
	BEGIN 
		...
		set @tmpsql = @tmpsql + ', CONSTRAINT FK_' + LEFT(@tablename,128-15) + '_StateChange FOREIGN KEY (State) REFERENCES ' + @state + ' (StateId)'
		set @tmpsql = @tmpsql + ')'
	...
	*****************************/
	...
	/***************************** 
		ATTACH TRIGGERS
	*****************************/
	--EXEC tsc.CreateStateChangeTableTriggers @schemaname, @tablename
END
GO
/****** Object:  StoredProcedure [tsc].[RebuildStateChangeTableTriggers]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [tsc].[RebuildStateChangeTableTriggers]
AS BEGIN 
	declare @tmpsql as varchar(max)
	;with TriggerAction as ( 
		SELECT	 'DROP TRIGGER [' + s.name + '].[' + o.name + '];' [Remove]
				,'EXEC tsc.CreateStateChangeTableTriggers @schemaname = ''input'', @tablename = ''' + t.name + ''';' [ReAdd]
		FROM sys.objects		o
...
END
GO
/****** Object:  DdlTrigger [HoldStateChange]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TRIGGER [HoldStateChange] ON DATABASE
AFTER DROP_TABLE
AS   
	declare @schemaname as varchar(128) = 'input'
	;IF EVENTDATA().value('(/EVENT_INSTANCE/SchemaName)[1]','varchar(128)') <> @schemaname RETURN;
	PRINT '============= HoldStateChange ================'
	declare @tablename as varchar(128) = EVENTDATA().value('(/EVENT_INSTANCE/ObjectName)[1]','nvarchar(max)')

	EXEC tsc.HoldStateChange @tablename;
GO
DISABLE TRIGGER [HoldStateChange] ON DATABASE
GO
/****** Object:  DdlTrigger [ManifestStateChange]    Script Date: 25/04/2020 10:57:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TRIGGER [ManifestStateChange] ON DATABASE
AFTER CREATE_TABLE
AS   
	declare @schemaname as varchar(128) = 'input'
	;IF EVENTDATA().value('(/EVENT_INSTANCE/SchemaName)[1]','varchar(128)') <> @schemaname RETURN;
	PRINT '============= ManifestStateChange ================'
	declare @tablename as varchar(128) = EVENTDATA().value('(/EVENT_INSTANCE/ObjectName)[1]','nvarchar(max)')

	EXEC tsc.ManifestStateChange @schemaname, @tablename;
	/***************************** 
		ATTACH TRIGGERS
	*****************************/
	EXEC tsc.CreateStateChangeTableTriggers @schemaname, @tablename
GO
DISABLE TRIGGER [ManifestStateChange] ON DATABASE
GO
ENABLE TRIGGER [HoldStateChange] ON DATABASE
GO
ENABLE TRIGGER [ManifestStateChange] ON DATABASE
GO
