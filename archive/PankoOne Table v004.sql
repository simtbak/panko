...
/*------------------------------------------------------------------------------------------------------------
	--DROP PANKO
------------------------------------------------------------------------------------------------------------*/
IF EXISTS (SELECT * FROM sys.schemas WHERE name = 'tsc')
BEGIN
	/* -- DROP FUNCTIONS, VIEWS, TRIGGERS, AND PROCEDURES -- */
	declare @tmpsql as varchar(max);
	with HarvestTables as ( 
		SELECT	 'DROP ' 
				+ CASE o.type --COLLATE Latin1_General_CI_AS 
					WHEN 'U' THEN 'TABLE'
					WHEN 'V' THEN 'VIEW'
				...
				END
				+ ' [' + s.name + '].[' + o.name + '];' [Remove]
		FROM sys.objects		o
		INNER JOIN sys.schemas	s on s.schema_id = o.schema_id
		WHERE s.name in ('tsc','delta','state','report')	
		and o.type not in ('FK','PK')
	) 

	SELECT @tmpsql = (
		SELECT x 
			FROM ( 
						SELECT [Remove] x	FROM HarvestTables GROUP BY [Remove]
			) actions
	FOR XML PATH(''), TYPE).value('.', 'varchar(max)')
	EXEC(@tmpsql);
	
	/* -- DROP TARGET TABLE PROCEDURES -- */
